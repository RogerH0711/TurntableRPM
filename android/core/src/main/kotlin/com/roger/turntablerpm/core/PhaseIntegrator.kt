...
class PhaseIntegrator {
    ...
    /** 未經重設的陀螺儀積分總角度。 */
    var gyroTotalDegrees: Double = 0.0
        private set

    /** 磁北參考量到的總轉角（絕對值）。這是不受陀螺儀比例因子影響的獨立量測。 */
    var magneticTotalDegrees: Double = 0.0
        private set
    ...
    private var unwrappedYaw: Double? = null
    private var lastYaw: Double? = null
    private var yawAtAnchor: Double = 0.0

    fun add(sample: SpinSample) {
        lastTime?.let { previousTime ->
            val step = (sample.t - previousTime) * (sample.omega + lastOmega) / 2.0
            angleDegrees += step
        ...
        val yaw = sample.yaw ?: return

        val previousYaw = lastYaw
        val accumulatedSoFar = unwrappedYaw
        if (previousYaw != null && accumulatedSoFar != null) {
            var delta = yaw - previousYaw
            while (delta > PI) delta -= 2 * PI
            while (delta < -PI) delta += 2 * PI
            val accumulated = accumulatedSoFar + delta
            unwrappedYaw = accumulated
            magneticTotalDegrees = abs(accumulated) * 180.0 / PI

            val travelled = accumulated - yawAtAnchor
            if (abs(travelled) >= 2 * PI) {
                revolutions += 1
                ...
                //    合成訊號原本只產生遞增的 yaw，所以整組測試全過卻在真機上炸掉
                //    （4921 個樣本卻報 4730 圈）。
                yawAtAnchor += if (travelled < 0) -2 * PI else 2 * PI
                angleDegrees = revolutions * 360.0
            }
        } else {
            unwrappedYaw = 0.0
            yawAtAnchor = 0.0
     ...
     *
     * **要跑滿數十圈才可信**，而且兩條路徑分歧量不夠大時這個數字無法採信 —— 見 iOS 端的坑 11。
     */
    val calibrationEstimate: Double?
        get() = if (revolutions >= 1 && gyroTotalDegrees > 0 && magneticTotalDegrees > 0) {
            magneticTotalDegrees / gyroTotalDegrees
        } else null
}
